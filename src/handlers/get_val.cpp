...
get_val_handler::get_val_handler(storage::storage&& storage, const std::string& path) : _storage(std::move(storage)), handler_path(path) {}
...
seastar::future<std::unique_ptr<seastar::http::reply> > get_val_handler::handle(const seastar::sstring& path,
    ...
    try {
        ...
        auto value = co_await _storage.get_val(key_str);
        if (!value) {
